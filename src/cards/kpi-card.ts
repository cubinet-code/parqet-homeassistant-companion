...
export class ParqetKpiCard extends LitElement {
  ...
  static getStubConfig(): Partial<KpiCardConfig> {
    return { data_source: 'rest', kpi: 'total_value', default_interval: '1y', currency_symbol: '€' };
  }
  ...
  private async _loadData(silent = false) {
...
class ParqetKpiCardEditor extends LitElement {
  ...
  setConfig(config: KpiCardConfig): void {
    ...
    this._config = c;
    this._connected = oauthManager.isTokenValid(c.client_id);
