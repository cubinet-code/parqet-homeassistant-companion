 ...
 * Uses the Web Crypto API when available, with a pure-JS SHA-256 fallback
 * for non-secure contexts (HTTP) where crypto.subtle is undefined.
 */
...
function sha256Fallback(data: Uint8Array): Uint8Array {
  ...
  // message + 0x80 byte + padding zeros + 8-byte length
  const totalLen = Math.ceil((data.length + 9) / 64) * 64;
  const padded = new Uint8Array(totalLen);
  padded.set(data);
  padded[data.length] = 0x80;
  // Write length as big-endian 64-bit (only lower 32 bits needed for our use case)
